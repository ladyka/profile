...
public class PostService {
    ...
    public PostViewDto save(String postId, String username, String description) {
        UserEntity owner = userEntityRepository.findByUsername(username).orElseThrow();
        PostEntity blankPost = new PostEntity();
        blankPost.setOwnerId(String.valueOf(owner.getId()));
        PostEntity post = (postId != null)
                          ? postRepository.findById(postId).orElse(blankPost)
                          : blankPost;
        post.setDescription(description);
        postRepository.save(post);
        return postMapper.toDto(post, owner.getNickname(), owner.getZoneId());
    }
